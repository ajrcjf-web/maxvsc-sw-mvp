"""
Escenarios 'damped' de ejemplo usando parámetros con amortiguamiento:

1) Paso pequeño de potencia en modo PQ.
2) Paso pequeño de id_ref en modo VdcQ.
...
Uso desde la raíz del repo:
    ...
    $env:PYTHONPATH='src'; python examples/step_damped.py
"""
...
def run_pq_damped(repo_root: Path) -> None:
    params_path = repo_root / "examples" / "params_damped.json"
    scenario_path = repo_root / "examples" / "scenario_pq_damped.json"
    ...
    print("=== Escenario damped: paso de potencia PQ ===")
    print(f"  t_end       = {t_end}")
...
def run_vdcq_damped(repo_root: Path) -> None:
    params_path = repo_root / "examples" / "params_damped.json"
    scenario_path = repo_root / "examples" / "scenario_vdcq_damped.json"
    ...
    print("=== Escenario damped: paso de id_ref (VdcQ) ===")
    print(f"  t_end       = {t_end}")
    print(f"  t_step      = {step_time}")
...
def main():
    ...
    run_pq_damped(repo_root)
    run_vdcq_damped(repo_root)
