"""
Caso avanzado: variación de Vdc_ref (modo Vdc/Q).

Escenario:
- Vdc_ref = V0 en el primer tramo.
- Vdc_ref = V1 en el segundo tramo.
...
- Se ejecutan dos simulaciones independientes usando run_simulation:
    1) Segmento 1: [0, t_step] con Vdc_ref = V0
    2) Segmento 2: [0, t_end - t_step] con Vdc_ref = V1
...
def _get_project_root() -> Path:
    """
    Asumimos este archivo en: examples/advanced/case_vary_vdc.py
    -> raíz del proyecto está dos niveles arriba.
    """
    return Path(__file__).resolve().parents[2]


def _set_vdc_ref(params_config: Dict[str, Any],
                 scenario_config: Dict[str, Any],
                 Vdc_ref: float) -> None:
    """
    Coloca Vdc_ref en params_config o scenario_config según exista la clave.
...
def run_case_vary_vdc(
    ...
    """
    Ejecuta el caso avanzado de variación en Vdc_ref.

    Devuelve:
    - results_seg1: resultados de run_simulation con Vdc_ref = V0, [0, t_step]
    - results_seg2: resultados de run_simulation con Vdc_ref = V1, [0, t_end - t_step]
    """
    ...
    # --- Segmento 1: Vdc_ref = V0, [0, t_step] ---
    _set_vdc_ref(params_config_1, scenario_config_1, V0)
    ...
    # --- Segmento 2: Vdc_ref = V1, [0, t_end - t_step] ---
    _set_vdc_ref(params_config_2, scenario_config_2, V1)
...
def plot_results(results_seg1: Dict[str, Any],
    ...
    # Corrientes
    plt.figure()
    plt.plot(t1, id1, label="id (V0)")
    plt.plot(t1, iq1, label="iq (V0)")
    plt.plot(t2, id2, "--", label="id (V1)")
    plt.plot(t2, iq2, "--", label="iq (V1)")
    plt.xlabel("t [s]")
    ...
    # Vdc
    plt.figure()
    plt.plot(t1, vdc1, label="Vdc (V0)")
    plt.plot(t2, vdc2, "--", label="Vdc (V1)")
    plt.xlabel("t [s]")
    plt.ylabel("Vdc [pu]")
    ...
    if any(p1) or any(p2) or any(q1) or any(q2):
        plt.figure()
        if any(p1):
            plt.plot(t1, p1, label="P_ac (V0)")
        if any(p2):
            plt.plot(t2, p2, "--", label="P_ac (V1)")
        if any(q1):
            plt.plot(t1, q1, label="Q_ac (V0)")
        if any(q2):
            plt.plot(t2, q2, "--", label="Q_ac (V1)")
        plt.xlabel("t [s]")
...
def main() -> None:
    ...
    res1, res2 = run_case_vary_vdc(V0=V0, V1=V1, t_step=t_step, t_end=t_end, dt=dt)

    plot_results(
        res1,
        res2,
        t_step=t_step,
        title_suffix=f"(Vdc_ref: {V0} -> {V1} en t={t_step}s)",
    )
    export_results(res1, res2, name="vary_vdc")
