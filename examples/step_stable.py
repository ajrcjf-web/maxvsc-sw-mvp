"""
Escenarios "estables" de ejemplo:

1) Paso pequeño de potencia en modo PQ.
2) Paso pequeño de id_ref en modo VdcQ (emulando cambio de Vdc).
...
Uso:
    ...
    $env:PYTHONPATH='src'; python examples/step_stable.py
"""
...
def run_pq_small_step(repo_root: Path) -> None:
    params_path = repo_root / "examples" / "params_stable.json"
    scenario_path = repo_root / "examples" / "scenario_pq_small_step.json"
    ...
    print("=== Escenario estable: paso pequeño de potencia PQ ===")
    print(f"  t_end       = {t_end}")
...
def run_vdcq_small_step(repo_root: Path) -> None:
    params_path = repo_root / "examples" / "params_stable.json"
    scenario_path = repo_root / "examples" / "scenario_vdcq_small_step.json"
    ...
    print("=== Escenario estable: paso pequeño de id_ref (VdcQ) ===")
    print(f"  t_end       = {t_end}")
    print(f"  t_step      = {step_time}")
...
def main():
    ...
    run_pq_small_step(repo_root)
    run_vdcq_small_step(repo_root)
