"""
Escenarios 'damped_pretty' de ejemplo usando parámetros con amortiguamiento:

1) Paso de potencia en modo PQ (pequeño, pero más visible que en ultrastable).
...
Uso desde la raíz del repo:
    ...
    $env:PYTHONPATH='src'; python examples/step_damped_pretty.py
"""
...
def run_pq_damped_pretty(repo_root: Path) -> None:
    """
    Escenario en modo PQ con paso de potencia moderado.
    """
    params_path = repo_root / "examples" / "params_damped_pretty.json"
    scenario_path = repo_root / "examples" / "scenario_pq_damped_pretty.json"
    ...
    print("=== Escenario damped_pretty: paso de potencia PQ ===")
    print(f"  t_end       = {t_end}")
...
def run_vdcq_damped_pretty(repo_root: Path) -> None:
    """
    Escenario en modo VdcQ con paso de id_ref moderado.
    """
    params_path = repo_root / "examples" / "params_damped_pretty.json"
    scenario_path = repo_root / "examples" / "scenario_vdcq_damped_pretty.json"
    ...
    print("=== Escenario damped_pretty: paso de id_ref (VdcQ) ===")
    print(f"  t_end       = {t_end}")
    print(f"  t_step      = {step_time}")
...
def main():
    ...
    run_pq_damped_pretty(repo_root)
    run_vdcq_damped_pretty(repo_root)
